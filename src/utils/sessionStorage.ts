...
export interface UIPreferences {
  ...
  currentLayout?: "standard" | "compactList" | "wideView" | "pivotTable";
  lastActiveFilters?: any;
}

// Generic sessionStorage wrapper with error handling
class SessionStorageManager {
  ...
  public getItem<T>(key: string, defaultValue: T): T {
    if (!this.isAvailable()) {
      console.warn("sessionStorage is not available, using default value");
      return defaultValue;
    }
...
export const clearUIPreferences = (): boolean => {
  console.warn("Clearing UI preferences from sessionStorage");
  return storageManager.removeItem(STORAGE_KEYS.UI_PREFERENCES);
};

export const clearAllData = (): boolean => {
  console.warn("Clearing all assortment planner data from sessionStorage");
  return (
